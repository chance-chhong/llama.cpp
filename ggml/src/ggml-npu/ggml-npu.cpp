...
static void ggml_backend_npu_out_prod(ggml_backend_npu_context * ctx, struct ggml_tensor * dst) {
    const struct ggml_tensor * src0 = dst->src[0];
    const struct ggml_tensor * src1 = dst->src[1];
    ...
    for (int i = 0; i < cgraph->n_nodes; i++) {
        ...
        switch (node->op) {
            ...
            case GGML_OP_OUT_PROD:
                ggml_backend_npu_out_prod(ctx, node);
                break;
...
static ggml_guid_t ggml_backend_npu_guid(void) {
    static ggml_guid guid = { 0x88, 0x77, 0x66, 0x55, 0x44, 0x33, 0x22, 0x11, 0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0xff, 0x00, 0x99 };
    return &guid;
}
...
void ggml_backend_npu_set_n_threads(ggml_backend_t backend_npu, int n_threads) {
    GGML_ASSERT(ggml_backend_is_npu(backend_npu));

    ggml_backend_npu_context * ctx = (ggml_backend_npu_context *)backend_npu->context;
...
static void * ggml_backend_npu_get_proc_address(ggml_backend_reg_t reg, const char * name) {
    if (std::strcmp(name, "ggml_backend_set_n_threads") == 0) {
        return (void *)ggml_backend_npu_set_n_threads;
    }
    return NULL;
